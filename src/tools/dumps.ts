...
export function registerDumpsTools(server: McpServer, herdConfig: HerdConfig, runner: CliRunner): void {
  ...
  // ── Telescope (optional package) ──────────────────────────────────────────

  server.tool('telescope_install',
    'Install Laravel Telescope debugging dashboard (composer require laravel/telescope --dev)',
    {
      cwd: z.string().describe('Laravel project root directory'),
    },
    async ({ cwd }) => {
      try {
        ...
        const migrate = runner.php(['artisan', 'migrate'], cwd);
        lines.push('=== migrate ===\n' + (migrate.stdout || migrate.stderr));

        lines.push('\nTelescope installed. Visit /telescope on your site to open the dashboard.');
  ...
  server.tool('telescope_status',
    'Show Telescope recording status and entry counts (php artisan telescope:status)',
    {
      cwd: z.string().describe('Laravel project root directory'),
    },
    async ({ cwd }) => {
      try {
        const result = runner.php(['artisan', 'telescope:status'], cwd);
        return textResult(result.stdout || result.stderr || 'Telescope not installed.');
      } catch (e) { return errorResult(e); }
    }
  );
  ...
  server.tool('telescope_prune',
    'Delete old Telescope entries (php artisan telescope:prune)',
    {
