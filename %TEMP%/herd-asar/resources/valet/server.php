...
register_shutdown_function(function () use ($valetSitePath) {
    $configPath = $_SERVER['HERD_HOME'].'/config/';
    if (file_exists(($configPath.'/herd.json'))) {
        $existingConfig = @file_get_contents($configPath.'/herd.json');
        try {
    ...
    try {
        ...
        if(!(array_key_exists('herd', $_GET) && $_GET['herd'] === 'preview')) {
            $config = array_merge($existingConfig, [
                'lastSite' => $valetSitePath,
            ]);

            @file_put_contents($configPath.'/herd.json', json_encode($config));
        }
