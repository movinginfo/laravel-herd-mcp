...
if (! defined('VALET_HOME_PATH')) {
    if (testing()) {
        ...
        define('VALET_HOME_PATH', $_SERVER['HOME'].'/.config/herd/config/valet');
    }
}
if (! defined('HERD_HOME_PATH')) {
    define('HERD_HOME_PATH', $_SERVER['HOME'].'/.config/herd');
}
if (! defined('HERD_BIN_PATH')) {
    define('HERD_BIN_PATH', $_SERVER['HOME'].'/.config/herd/bin');
}
...
function silent($isSilent = true) {
    $container = Container::getInstance();

    if ($isSilent) {
        $container->instance('writer', new \Symfony\Component\Console\Output\NullOutput());
    } else {
        $container->instance('writer', new ConsoleOutput());
    }
}
...
/**
 * Output the given text to the console and abort the script.
 */
function abort(string $output): void
{
    warning($output);
...
function clear(int $lines = 1): void
{
    silent(false);
    // move cursor up n lines
    writer()->write("\x1b[{$lines}A");

    // erase to end of screen
    writer()->writeln("\x1b[0J");
