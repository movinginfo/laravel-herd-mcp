...
class Filesystem
{
    ...
    private function symlinkScript()
    {
        $script = '$link = $args[0]' . "\r\n";
        $script .= '$target = $args[1]' . "\r\n\r\n";
        $script .= 'New-Item -Path $link -ItemType SymbolicLink -Value $target';

        $filename = tempnam(sys_get_temp_dir(), 'symlink.ps1').'.ps1';

        $this->put($filename, $script);
    ...
    public function symlink(string $target, string $link): void
    {
        ...
        $script = $this->symlinkScript();

        $script = str_replace('/', '\\', $script);
        $command = sprintf('Start-Process PowerShell -ArgumentList \'-NoProfile -ExecutionPolicy Bypass -File ""%s"" ""%s"" ""%s""\' -Verb RunAs -WindowStyle Hidden', $script, $link, $target);

        CommandLineFacade::powershell($command, function ($code, $output) {
            error("Failed to create symbolic link: $output");
        });
    }
